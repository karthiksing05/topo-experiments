"""Evaluation script for train_topo_group_lasso experiments.

Loads the four trained model checkpoints (topo_gl, topo_kl_ent, topo_only,
baseline) and generates all visualisation figures under
outputs/train_topo_group_lasso/:
    ...
    debug/         – debug cortical-sheet figures (activation distribution + GL/KL/ent values)
...
python examples/test/test_topo_group_lasso.py \\
    ...
    --checkpoint-base         outputs/train_topo_group_lasso/checkpoints/best_baseline.pt
...
from train_topo_group_lasso import (        # shared definitions
    BASE_DIR,
    SimpleNN,
    FMNIST_CLASSES,
    VIS_LAYER_NAMES,
...
_DEFAULT_CONFIG = {
    ...
    "layers": {
        "fc1": {
            ...
            "lambda_gl":      1.0,
            "lambda_kl":      0.0, "lambda_entropy": 1.0, "temperature": 3.0,
        },
    },
    # checkpoint overrides (None → auto-detected from ckpt_dir)
    "checkpoint_topo_gl":     None,
    "checkpoint_topo_kl_ent": None,
...
def get_config() -> dict:
    p = argparse.ArgumentParser(
        description="Evaluate / visualise trained train_topo_group_lasso checkpoints.",
    ...
    p.add_argument("--checkpoint-topo-gl",       type=str, default=None,
                   help="Path to topo_gl checkpoint")
    p.add_argument("--checkpoint-topo-kl-ent",   type=str, default=None,
                   help="Path to topo_kl_ent checkpoint")
    p.add_argument("--checkpoint-topo-only",     type=str, default=None,
                   help="Path to topo_only checkpoint")
    ...
    cli_map = {
        ...
        "device":                cli.device,
        "checkpoint_topo_gl":    cli.checkpoint_topo_gl,
        "checkpoint_topo_kl_ent": cli.checkpoint_topo_kl_ent,
...
def evaluate(cfg: dict) -> None:
    ...
    topo_gl_ckpt     = _resolve_ckpt("checkpoint_topo_gl",     "best_topo_gl.pt")
    topo_kl_ent_ckpt = _resolve_ckpt("checkpoint_topo_kl_ent", "best_topo_kl_ent.pt")
    ...
    print("=" * 65)
    topo_gl_model     = _load_model(topo_gl_ckpt,     hidden_size, device)
    topo_kl_ent_model = _load_model(topo_kl_ent_ckpt, hidden_size, device)
    topo_only_model   = _load_model(topo_only_ckpt,   hidden_size, device)
    base_model        = _load_model(base_ckpt,        hidden_size, device)

    all_models = {
        "topo_gl":     topo_gl_model,
        "topo_kl_ent": topo_kl_ent_model,
        "topo_only":   topo_only_model,
        "baseline":    base_model,
    }
    # Topo-only variants (exclude baseline) for some comparisons
    topo_models = {
        "topo_gl":     topo_gl_model,
        "topo_kl_ent": topo_kl_ent_model,
        "topo_only":   topo_only_model,
    }
    # Sparsity comparison: the two penalised topo variants
    sparsity_models = {
        "topo_gl":     topo_gl_model,
        "topo_kl_ent": topo_kl_ent_model,
    }
    ...
    # Subset comparisons — group-lasso vs KL/entropy (sparsity comparison)
    save_subset_comparison_figure(
        sparsity_models, val_loader, selectivity_dir, device,
        class_indices=[0, 2, 4, 6],
        tag="tops_sparsity",
        subtitle="Tops — Group-Lasso vs KL/Entropy",
    )
    save_subset_comparison_figure(
        sparsity_models, val_loader, selectivity_dir, device,
        class_indices=[5, 7, 9, 8],
        tag="shoes_bag_sparsity",
        subtitle="Shoes & Bag — Group-Lasso vs KL/Entropy",
    )

    # Subset comparisons — all topo variants
    save_subset_comparison_figure(
...
if __name__ == "__main__":
    ...
    log_path = log_dir / f"test_topo_group_lasso-{ts}.txt"
