#!/bin/bash
#SBATCH --job-name=eval_tox_strats_125m
...
#SBATCH --output=slurm/slurm_outputs/eval_tox_strats_125m-%j.out
#SBATCH --error=slurm/slurm_errors/eval_tox_strats_125m-%j.err

# Evaluate NEW strategies (EigenShift, Self-Debiasing, CHaRS-lite, Vocab Shifting, PCT)
# on topo-nanoGPT 125M checkpoints.
#
# Configurable knobs (override via: sbatch --export=ALL,VAR=value ...):
#   TAUS, FRACS, N_PROMPTS, N_GEN, MAX_NEW_TOKENS, TEMPERATURE, TOP_K,
#   N_SELECTIVITY_TOKENS, NO_EIGENSHIFT, NO_SELF_DEBIASING, NO_CHARS,
#   NO_VOCAB_SHIFTING, NO_PCT, NO_LLAMAGUARD, NO_TOXIGEN, NO_RTP,
#   RESUME, OUTPUT_DIR, FRAC_MAX
...
NO_RTP="${NO_RTP:-0}"
RESUME="${RESUME:-0}"
OUTPUT_DIR="${OUTPUT_DIR:-${EXPERIMENT_DIR}/outputs/toxicity_strategies_nanogpt}"

# FRAC_MAX: auto-generate fracs list to avoid SLURM comma issues
...
# -- Job info ------------------------------------------------------------------
echo "=============================================="
echo "Topo NanoGPT 125M -- New Strategies Eval"
echo "=============================================="
...
# -- Build cmd -----------------------------------------------------------------
CMD=(
    python -u -m src.toxicity.eval_toxicity_strategies_nanogpt
    --taus "$TAUS"
    --fracs "$FRACS"
    --n_prompts "$N_PROMPTS"
    --n_gen "$N_GEN"
