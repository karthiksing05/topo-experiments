#!/usr/bin/env bash
# ────────────────────────────────────────────────────────────────────────────
# replot_toxicity_techniques_nanogpt_450m.sh
# Re-generate all technique-comparison toxicity plots from saved JSON results
# for the 450M models. No GPU required — pure matplotlib / numpy.
#
# Usage:
#   sbatch scripts/toxicity/replot_toxicity_techniques_nanogpt_450m.sh
#
# Override input directory:
#   sbatch --export=ALL,INPUT_DIR=/path/to/dir \
#          scripts/toxicity/replot_toxicity_techniques_nanogpt_450m.sh
#
# Skip subsets:
#   sbatch --export=ALL,NO_SELECTIVITY=1,NO_CORTICAL=1 \
#          scripts/toxicity/replot_toxicity_techniques_nanogpt_450m.sh
# ────────────────────────────────────────────────────────────────────────────
#SBATCH --job-name=replot_tox_tech_450m
#SBATCH --exclude=spot,heistotron,clippy,hal,asimo,kipp,smith,t1000,bb8,jarvis,gideon,ripl-s1,ash,c3po,calculon,eva,johnny5,neo,tars,vicki,ava,jill,walle
#SBATCH --output=slurm/slurm_outputs/replot_tox_tech_450m-%j.out
#SBATCH --error=slurm/slurm_errors/replot_tox_tech_450m-%j.err
...
# ── Configurable knobs ────────────────────────────────────────────────────────
INPUT_DIR="${INPUT_DIR:-${EXPERIMENT_DIR}/outputs/toxicity_techniques_nanogpt_450m}"
...
echo "=============================================="
echo "Replot: toxicity techniques (450M)"
echo "=============================================="
echo "input_dir  : $INPUT_DIR"
echo "output_dir : ${OUTPUT_DIR:-(same as input)}"
echo "=============================================="

srun python -u src/toxicity/replot_toxicity_techniques_nanogpt_450m.py "${ARGS[@]}"
