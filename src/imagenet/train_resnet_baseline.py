"""
Train ResNet-18 on ImageNet with cross-entropy only (no regularisation).

Variant: baseline
  Loss = CrossEntropy only.  No TopoLoss, no entropy penalty.

Requires FFCV .beton files written by write_imagenet_ffcv.py.

Usage
-----
python src/train/train_resnet_baseline.py
python src/train/train_resnet_baseline.py \\
...
def train(cfg: dict) -> None:
    print("=" * 70)
    print("  BASELINE — CrossEntropy only (no TopoLoss, no entropy penalty)")
    print("=" * 70)
    ...
    run_training(
        ...
        residual_convs={},
        layer_cfg={},
        default_layer_cfg=cfg["default_layer_cfg"],
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Train ResNet-18 baseline — CrossEntropy only (FFCV).",
