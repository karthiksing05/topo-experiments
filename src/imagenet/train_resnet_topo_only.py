"""
Train ResNet-18 on ImageNet with TopoLoss only (no entropy-sparsity penalty).

Variant: topo_only
  Loss = CrossEntropy + TopoLoss (LaplacianPyramid on every targeted
         residual-block conv).  lambda_entropy is forced to 0.

Requires FFCV .beton files written by write_imagenet_ffcv.py.

Usage
-----
python src/train/train_resnet_topo_only.py
python src/train/train_resnet_topo_only.py \\
...
def train(cfg: dict) -> None:
    print("=" * 70)
    print("  TOPO ONLY — TopoLoss on all residual convs (entropy penalty zeroed)")
    print("=" * 70)
    ...
    run_training(
        ...
        layer_cfg=no_entropy_layer_cfg,
        default_layer_cfg=no_entropy_default_lc,
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Train ResNet-18: TopoLoss only, no entropy penalty (FFCV).",
