"""
Train ResNet-18 + TopoLoss + L1-entropy sparsity penalty on STL-10,
then finetune on CIFAR-10, measuring catastrophic forgetting.

Sparsity objective: minimise H(|x| / ||x||_1) on the downsampled cortical
sheet.  Unlike softmax-based entropy, zeros contribute zero probability mass,
so the model is directly rewarded for silencing cortical units.

Only lambda_sparse is active; lambda_entropy and lambda_kl are forced to 0.

Output: outputs/stl_cifar/topo_l1sparse_results_latest.json
"""
...
def _setup_l1sparse(cfg: dict) -> tuple[dict, dict]:
    """Zero lambda_entropy and lambda_kl; keep lambda_sparse for the L1-entropy loss."""
    default_lc = {
    ...
    layer_cfg = {
        name: {**lc, "lambda_entropy": 0.0, "lambda_kl": 0.0}
        for name, lc in cfg["layers"].items()
    }
    return layer_cfg, default_lc


def train(cfg: dict) -> None:
    print("=" * 70)
    print("  TOPO + L1-SPARSE — TopoLoss + L1-entropy sparsity on STL→CIFAR")
    print("=" * 70)
    ...
    layer_cfg, default_lc = _setup_l1sparse(cfg)
    topo_loss = build_topo_loss(model, residual_convs, layer_cfg, default_lc)

    lam_sparse = default_lc.get("lambda_sparse", 0.0)
    print(f"  lambda_sparse = {lam_sparse}  (lambda_entropy=0, lambda_kl=0)")
...
def main():
    p = argparse.ArgumentParser(description="TopoLoss+L1-sparse STL→CIFAR forgetting experiment")
