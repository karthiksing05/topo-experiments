"""
Train ResNet-18 + TopoLoss on STL-10, then finetune on CIFAR-10.
Entropy-sparsity penalty is zeroed out — topology structure only.

Output: outputs/stl_cifar/topo_only_results_latest.json
"""
...
def _zero_sparsity(cfg: dict) -> tuple[dict, dict]:
    """Return layer_cfg and default_layer_cfg with all sparsity penalties forced to 0."""
    default_lc = {**cfg["default_layer_cfg"], "lambda_entropy": 0.0, "lambda_kl": 0.0, "lambda_sparse": 0.0}
    layer_cfg  = {
        name: {**lc, "lambda_entropy": 0.0, "lambda_kl": 0.0, "lambda_sparse": 0.0}
...
def train(cfg: dict) -> None:
    print("=" * 70)
    print("  TOPO ONLY — TopoLoss on residual convs, entropy penalty zeroed")
    print("=" * 70)
    ...
    layer_cfg, default_lc = _zero_sparsity(cfg)
...
def main():
    p = argparse.ArgumentParser(description="TopoLoss-only STL→CIFAR forgetting experiment")
