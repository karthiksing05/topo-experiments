"""
Train ResNet-18 + TopoLoss + entropy-sparsity penalty on STL-10,
then finetune on CIFAR-10, measuring catastrophic forgetting.

Output: outputs/stl_cifar/topo_sparsity_results_latest.json
"""
...
def _zero_l1sparse(cfg: dict) -> tuple[dict, dict]:
    """Zero lambda_sparse so topo_sparsity uses only entropy+KL, not L1-entropy."""
    default_lc = {**cfg["default_layer_cfg"], "lambda_sparse": 0.0}
    layer_cfg  = {
        name: {**lc, "lambda_sparse": 0.0}
...
def train(cfg: dict) -> None:
    print("=" * 70)
    print("  TOPO + SPARSITY — TopoLoss + cortical-entropy penalty on STL→CIFAR")
    print("=" * 70)
    ...
    layer_cfg, default_lc = _zero_l1sparse(cfg)
...
def main():
    p = argparse.ArgumentParser(description="TopoLoss+sparsity STL→CIFAR forgetting experiment")
